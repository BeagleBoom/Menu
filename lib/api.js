...
const {AutoPersistSettings, WaveSettingsFile, AudioFilesystem} = require("./fileHandler");


let globalStore = {};
let settingsFile = path.join(__dirname, "/../", "settings.json");
let downloadFolder = path.join(__dirname, "/../", "files");
let savedFolder = path.join(__dirname, "/../", "saved");
...
module.exports = (stack, sendEvent, display, sendView, sendCaptions) => {
    let out = {
        ...
        audioFilesystem: new AudioFilesystem( [ savedFolder]),
        popState(...args) {
            return stack.popState(...args);
        },
        popStateUntil(...args) {
            return stack.popStateUntil(...args);
        },
        pushState(...args) {
